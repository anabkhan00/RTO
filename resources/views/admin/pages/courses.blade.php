            ...
            <tbody>
                @foreach ($courses as $course)
                    <tr class="border-b font-medium text-xs hover:bg-gray-50">
                        ...
                        <td class="p-3 whitespace-nowrap">{{ $course->credit_hours ?? 'N/A' }}</td>
                        <td class="p-3 whitespace-nowrap">{{ Str::limit($course->description ?? 'N/A', 30) }}</td>
                        <td class="p-3 whitespace-nowrap">
                            <span class="px-2 py-1 rounded text-xs {{ $course->status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $course->status ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="p-3 whitespace-nowrap">
                            <button
                                onclick="editCourse({{ $course->id }}, '{{ $course->name }}', '{{ $course->code }}', '{{ $course->credit_hours }}', '{{ addslashes($course->description) }}', {{ $course->status ? 'true' : 'false' }})"
                            ...
                            <form id="delete-form-{{ $course->id }}" method="POST"
                                action="/admin/courses/{{ $course->id }}" class="hidden">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
