// Extracted from resources/views/rto/pages/students.blade.php
...
let studentsTable = $('#studentsTable').DataTable({
    ...
    "serverSide": true,
    "ajax": {
        "url": studentsPageConfig.dataUrl || "/rto/students/data",
        "type": "GET",
        "data": function (d) {
...
function updateStudentStatus(id, newStatus) {
    fetch(`/rto/students/update-status/${id}`, {
        method: 'POST',
        headers: {
...
function loadCoordinators() {
    fetch('/rto/students/coordinators')
        .then(response => response.json())
        .then(coordinators => {
...
document.getElementById('sourcingCoordinatorForm').addEventListener('submit', function (e) {
    ...
    fetch('/rto/student-assignments/bulk', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': getCsrfToken()
